...
又研究了一下，发现有固件安装和插件安装两种形式。固件安装支持ax6 [Index of /pub/fros/redmi_ax6/](http://175.178.71.82:88/pub/fros/redmi_ax6/)， 插件安装就是利用已经刷好的openwrt系统进行操作。
选择了插件安装形式，一通操作猛如虎之后，也tnnd没有屏蔽抖音，反而路由器经常重启。
我也不确定是op不稳定，还是刷了fros之后的op不稳定，虽然后者的可能性更大。

一气之下，利用不扩容分区的命令，从小米固件重启，先用着老的白。

# 暂时的结局（2023-05-22）
继续用着小米的官方固件。
在从openwrt恢复官方的时候，刚开始还出现了连接上了wifi不能登录后台的情况，也不能上网。
按了路由器上的reset键，重新登进去才好的。结果重新登陆后发现路由器自动更新到了最新系统。。。按照双分区的设计理念，那我之前装的openwrt就没了。。。
